...
SEARCH_PAGES = 3   # Number of search result pages to fetch
MIN_PLAYLISTS = 15  # Minimum number of playlists to find before starting collection
SLEEP = 0.3
    ...
    for kw in keywords:
        ...
        try:
            ...
            for page in range(SEARCH_PAGES):
                res = sp.search(q=f'"{kw}"', type="playlist", limit=SEARCH_LIMIT, offset=offset, market="US")
            ...
            if len(found) < MIN_PLAYLISTS:
                ...
                offset = 0
                for page in range(SEARCH_PAGES):
                    res = sp.search(q=kw, type="playlist", limit=SEARCH_LIMIT, offset=offset, market="US")
                    if not res or "playlists" not in res:
                        break
